...
public class PlayerTest {
    ...
    public void move() {
        ...
        testMap[pilotPos.yPos + 1][pilotPos.xPos].flood();
        testMap[pilotPos.yPos - 1][pilotPos.xPos].flood();
        destination = new Point(oldPilotPos);
        destination.setLocation(oldPilotPos.xPos + 1, oldPilotPos.yPos);
        pilot = action.getActivePlayer();
        assertTrue("Der Pilot hätte sich bewegen dürfen müssen", pilot.move(destination, true, false));
        assertEquals("Der Pilot hätte laufen müssen", destination, pilot.getPosition());
        pilot.move(oldPilotPos, false, false);
        ...
        oldPilotPos = new Point(pilotPos.getLocation());
        ...
        oldPilotPos = new Point(pilotPos.getLocation());
        destination = new Point(pilotPos.xPos, pilotPos.yPos + 1);
        pilot = action.getActivePlayer();
        assertFalse("Der Pilot hätte sich nicht bewegen dürfen", pilot.move(destination, true, false));
        assertEquals("Der Pilot hätte sich nicht bewegen dürfen", oldPilotPos, pilot.getPosition());
        assertEquals("", 2, pilot.getActionsLeft());
        ...
        oldPilotPos = new Point(pilotPos.getLocation());
        pilot.setPosition(MapUtil.getPlayerSpawnPoint(testMap, PlayerType.PILOT));
        pilot = action.getActivePlayer();
        destination = new Point(pilotPos.xPos + 1, pilotPos.yPos);
        assertFalse("Der Pilot hätte sich nicht bewegen dürfen", pilot.move(destination, true, false));
        assertEquals("Der Pilot hätte sich nicht bewegen dürfen", oldPilotPos, pilot.getPosition());
    ...
    public void drainablePositions() {
        ...
        Player navigator = action.getPlayers().get(1);
        Point navigatorPos = MapUtil.getPlayerSpawnPoint(testMap, PlayerType.NAVIGATOR);
       ...
       assertTrue("trockene Felder können nicht trockengelegt werden", drainablePositionsNavigator.isEmpty());
        ...
        //teste mit 4 FLOODED Feldern
        testMap[navigatorPos.yPos + 1][navigatorPos.xPos].flood();
        testMap[navigatorPos.yPos - 1][navigatorPos.xPos].flood();
        testMap[navigatorPos.yPos][navigatorPos.xPos + 1].flood();
        testMap[navigatorPos.yPos][navigatorPos.xPos - 1].flood();
        drainablePositionsNavigator = navigator.drainablePositions();
        for (Point currentPos : drainablePositionsNavigator) {
            assertTrue("Felder sind niemals versunken!", testMap[currentPos.yPos][currentPos.xPos].getState() != MapTileState.GONE);
            assertTrue("Felder sind niemals trocken!", testMap[currentPos.yPos][currentPos.xPos].getState() != MapTileState.DRY);
        }
        ...
        //teste mit Engineer Special Move
        ...
        // Teste mit Navigator, 2 null Feldern und einem GONE
        navigator.setPosition(new Point(7, 4));
        testMap[navigator.getPosition().yPos][navigator.getPosition().xPos].flood();
        drainablePositionsNavigator = navigator.drainablePositions();
        for (Point currentPos : drainablePositionsNavigator) {
            assertTrue("legale Felder sind niemals versunken!", testMap[currentPos.yPos][currentPos.xPos].getState() != MapTileState.GONE);
        }
